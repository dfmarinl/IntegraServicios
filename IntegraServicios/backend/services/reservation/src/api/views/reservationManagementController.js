...
const reservationManagementController = {
  ...
  getAllReservationsWithDetails: async (req, res) => {
    try {
      ...
      console.log('📋 Reservas detalladas llamadas con filtros:', {
        status, resourceId, userId, startDate, endDate, isRepetitive, page, limit
      });
        ...
        include: [
          ...
          },
          {
            ...
            include: [{
              ...
              include: [{
                model: Unit,
                attributes: ['id', 'name', 'description', 'granularity', 'isActive'] // ✅ Solo campos que EXISTEN
              }]
            }]
          }
        ],
    ...
    try {
      ...
      const reservation = await Reservation.findByPk(id, {
        include: [
          {
            model: User,
            attributes: ['id', 'firstName', 'lastName', 'email', 'rol', 'identificationNumber', 'city']
          },
          {
            model: Resource,
            attributes: ['id', 'name', 'photoUrl', 'features', 'isAvailable', 'isActive'],
            include: [{
              model: ResourceType,
      ...
      if (reservation.isRepetitive) {
        const repeatSeries = await Reservation.findAll({
          ...
          attributes: ['id', 'startDateTime', 'endDateTime', 'status', 'isRepetitive']
        });
  ...
  updateReservation: async (req, res) => {
    try {
      ...
      console.log('✏️ Actualizando reserva ID:', id, 'con datos:', { status, purpose, attendees });
      ...
      if ((startDateTime || endDateTime) && reservation.status === 'pendiente') {
        const newStart = startDateTime ? new Date(startDateTime) : new Date(reservation.startDateTime);
        const newEnd = endDateTime ? new Date(endDateTime) : new Date(reservation.endDateTime);
